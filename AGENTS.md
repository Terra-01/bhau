...
**Phase 2 in progress** (war room UI). `/` is a single-viewport terminal (desktop-first, the document never scrolls at xl; each tile owns its overflow): three column stacks composed directly in `src/app/page.tsx` with `minmax(0,·fr)` grid columns and `min-h-0` flex fractions. Data assembly: `src/lib/warroom.ts` + `src/lib/exchange.ts` + `src/lib/rates.ts` ("latest available" semantics — must render at any hour). Panels: watchlist (localStorage, 20 defaults incl. the indices, 10-row pages; hero chart on TradingView's lightweight-charts, Apache-2.0; quotes via `/api/quotes` — Yahoo live batch, archive fallback — 30s during the session; the chart auto-tours the list), India rates & macro (G-sec yield table + spreads + economy rows, TradingView table grammar), off the tape, stocks (most traded/gainers/losers + ETF section) in center with **the Floor** below it (hero, center column), intelligence (desk synthesis from `src/ingest/synthesize.ts`, needs OPENAI_KEY; wire refreshed at request time from the same RSS feeds, 15-min cache, pack fallback), right rail: city pulse (fuel incl. Pune / IBJA metals ⇄ weather) · forex vs INR (30d sparklines) · compact calendar (flat dated rows) · commodities · live TV (paused-by-default facade; chrome-less pointer-events-none embed always joins the live edge). Tabbed panels auto-rotate on staggered intervals via `src/lib/use-carousel.ts` (hover/hidden-tab/manual-hold aware); every data tile carries a computed one-line `InsightLine` (`src/components/insight-line.tsx`). **Live layer** (design/realtime-plan.md, built): `src/lib/market-clock.ts` + `src/lib/nse-live.ts` + `/api/live/{market,movers,health}` — NSE-direct with Yahoo fallback, 25s shared server caches, clients poll via `src/lib/use-live.ts` on a phase-aware cadence; `LiveChip` labels what's live; header `LiveTicker`; intraday stocks lists during the session; 1D minute chart via `/api/history/[symbol]?range=1d`. The Floor stays EOD by design. **Deployed**: https://bhau-one.vercel.app (Vercel project `terra01s-projects/bhau`, manual `vercel --prod` — repo not git-connected yet); prod health: NSE+Yahoo+Neon all reachable; YouTube live-id scrape blocked from datacenter IPs (TV tile degrades honestly). Charts run on lightweight-charts (`MarketChart`/`RaceChart`, themed via `src/lib/lw-theme.ts`) plus small custom SVGs (`src/components/sparkline.tsx`). Still to come (Phase 2b): `/m` mobile feed, panel customization UI, methodology page, OG cards.

**Phase 1** (agent loop, headless — running nightly). Strategy revision v2.0 (2026-09-03, `npm run floor:revision`, after v1.1 the same day): personas are abstract identities (Monsoon/Tape/Ballast/Undertow) with voices and per-persona evidence hierarchies; the pack carries `evidence` (src/ingest/evidence.ts — flows, rates, MMI via the new `mmi` fetcher, commodities, forex, breadth, sectors via src/lib/sectors.ts, street, calendar); deliberation gets a self-`mirror` (P&L, drawdown, tripwires — breaches machine-checked) and the contrarian sees colleagues' theses; BUYs carry structured invalidation, decisions may carry a watchlist, Fridays produce a public letter; payloads carry `promptVersion`. v1.1 (superseded): the pack carries a `quant` sheet (`src/ingest/quant.ts` — universe returns/52w distances/relative strength, backed by `src/ingest/backfill.ts` Yahoo history; rerun after quarterly universe refreshes), mandates carry deployment postures, and a NO_TRADE must publish its falsifiable trigger (`noTradeTrigger` → payload `trigger`). Cron runs 18:47/20:17/22:17 IST (slots follow publication times — bhavcopy/FII-DII ~18:00-19:00, AMFI NAV ~21:00+; deliberation defers until today's closes are in the pack or 21:00 IST) (GitHub scheduler jitters up to ~6h; floor is once-per-session idempotent). The floor tile ships the public record: per-agent popovers show book + full fill history + recent decisions (`floor.books`/`floor.log` in warroom.ts). The daily loop in `src/agents/` runs after ingest: fill yesterday's decisions at today's open → mark to market → deliberate for tomorrow (OpenAI Responses API, structured outputs, `gpt-5.6-luna`; the provider is isolated to `src/agents/deliberate.ts`, key in `OPENAI_KEY`). Everything lands on the append-only hash-chained ledger (`LedgerEntry`) — **there are no update/delete paths for ledger rows anywhere, ever**. No UI yet. Exit criterion: the loop runs unattended for a week.
