...
**Phase 2 in progress** (war room UI). `/` is a fixed-viewport 12×4 mosaic (desktop-first, page never scrolls at xl; tiles scroll internally): registry with explicit grid placement in `src/config/panels.ts`, data assembly in `src/lib/warroom.ts` ("latest available" semantics — must render at any hour), tiles in `src/components/tiles/`. Charts are vendored bklit-ui components (`src/components/charts/` — installed via shadcn registry, lint-ignored; visx + motion) — LineChart (equity race), Gauge (regime), Candlestick (Nifty). Daily desk synthesis ("What matters") is generated in the ingest pipeline (`src/ingest/synthesize.ts`, needs OPENAI_KEY). Design language per DESIGN.md — light broadsheet, tokens in `globals.css`, Geist Mono for every numeral. Still to come (Phase 2b): India map (bklit choropleth takes custom GeoJSON) + monsoon layer, `/m` mobile feed, panel customization UI, methodology page, OG cards.
