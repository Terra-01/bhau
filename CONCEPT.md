...
- ~~**City-wise retail prices (petrol/diesel/gold/silver)**~~ — **shipped 2026-09-01** as the City Pulse tile: metro petrol/diesel scraped from goodreturns (city-level via state taxes), gold/silver as IBJA's national AM/PM fix (bullion is a national benchmark in India — per-city quotes are marketing noise, so we don't fake a city dimension)
- **India rates & macro** — **shipped 2026-09-01**: G-sec yield curve (RBI homepage scrape: 91/182/364-day T-bill cut-offs + rotating benchmark bonds bucketed to 3/5/10/15/30Y), 10Y hero, 10Y−repo / 10Y−1Y / vs-US-10Y spreads, World Bank annual vitals (year-labeled). Known limits: corporate bond yields/spreads have no reliable keyless source (SEBI/exchange data is walled; retail CM prints are noise) — deferred honestly; CPI shown is the World Bank annual average, monthly MOSPI CPI has no keyless API
