...
**Build notes (same day):** shipped as `src/lib/market-clock.ts` (pure phase clock, unit-tested boundaries) + `src/lib/nse-live.ts` (warmed cookie jar, holiday-master cache) + routes `/api/live/market` (NSE allIndices + one Yahoo batch: indices/fx-crosses/commodities, 25s shared cache, stale-keep ladder), `/api/live/movers` (NSE live-analysis gainers/"loosers"/most-active, series-EQ filter, ₹-lakh turnover normalized), `/api/live/health`, upgraded `/api/quotes` (phase+source) and `/api/history/[symbol]?range=1d` (minute prints, last session only). Client: `useLive` hook (phase-aware cadence, hidden-tab pause), `LiveChip`, header `LiveTicker`, live overlays in movers/commodities/forex/watchlist, 1D tab on the lightweight-chart (IST-shifted timestamps). One plan change discovered in the field: **NSE removed `equity-stockIndices` (404), so watchlist quotes are Yahoo-primary** with the bhavcopy archive as fallback — allowed by the decided policy.

**Deploy day (2026-09-01, same evening):** production is live at **https://bhau-one.vercel.app** (project `terra01s-projects/bhau`, CLI-deployed — not yet git-connected, so deploys are manual `vercel --prod`). `/api/live/health` from the Vercel runtime: **NSE ok** (session-stamped), **Yahoo ok**, **Neon ok** — the open question is answered: NSE-direct works from Vercel egress today. Two deploy lessons fixed in code: the page no longer requires the database at build time (Vercel's builder couldn't reach Neon; a failed fetch renders the fallback and ISR replaces it at runtime), and env values must be pushed unquoted (`vercel env add` stores stdin verbatim — the `.env.local` quotes broke the URL once). Known limitation: YouTube blocks the `/api/tv` live-videoId scrape from datacenter IPs, so the deployed TV tile degrades to its off-air state with the open-on-YouTube link (fix candidates: YouTube Data API key, or resolving the id at ingest time from the home IP).
