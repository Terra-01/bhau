...
export function ForexRates({ forex }: { forex: ExchangeData["forex"] }) {
  return (
    <Tile title="Forex vs INR" meta="ECB REF · DAILY">
      {!forex ? (
            ...
            {forex.map((row) => {
              ...
              return (
                <tr key={row.pair} className="border-b border-ash last:border-b-0">
                  <td className="px-2.5 py-[2.5px] font-mono text-[10px] font-semibold text-charcoal">
                    {row.pair}
                  ...
                  <td className="px-1 py-[2.5px] text-right font-mono text-[10.5px] font-medium tabular-nums text-ink">
                    {fmt.format(row.price * scale)}
                  </td>
                  <td className={`px-2.5 py-[2.5px] text-right font-mono text-[9.5px] tabular-nums ${row.m1 !== null ? deltaClass(row.m1) : "text-fog"}`}>
